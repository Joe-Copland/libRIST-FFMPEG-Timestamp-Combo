...
//Used to check if we've already set the environment variable
int set_env = 0;
				...
				switch(payload.type) {
					...
					case RIST_PAYLOAD_TYPE_DATA_RAW:
						...
						if (RIST_UNLIKELY(p->config.timing_mode == RIST_TIMING_MODE_ARRIVAL))
							...
							//printf("MPEG RTP ts %u NTP ts %lu (approximated)\n",rtp_time,recovered_ntp_ts);
						}
